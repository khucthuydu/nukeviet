...
if ($rowcontent['id'] > 0) {
    ...
    $rowcontent['catids'] = $db->query("SELECT array_catid FROM " . $db_config['prefix'] . "_" . $module_data . "_rows where id=" . $rowcontent['id'])->fetchColumn();
    ...
    if (empty($error)) {
        ...
        if ($rowcontent['id'] == 0 or $is_copy) {
            ...
            if ($rowcontent['id'] > 0) {
                ...
                // // insert cats
                // $test = $db_config['prefix'] . '_' . $module_data.'_rows' . ' SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_rows WHERE id=' . $rowcontent['id'];
                // var_dump($test);die;

                // foreach ($catids as $catid) {
                //     $ct_query[] = (int) $db->exec('INSERT INTO ' .$db_config['prefix'] . '_' . $module_data.'_rows' . ' SELECT * FROM ' . $db_config['prefix'] . '_' . $module_data . '_rows WHERE id=' . $rowcontent['id']);
                // }
