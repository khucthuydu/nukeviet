...
if ($order_id > 0 and $checkss == md5($order_id . $global_config['sitekey'] . session_id())) {
    ...
    if (!empty($listid)) {
        foreach ($listid as $dbkey => $proid) {
            ...
            t1.' . NV_LANG_DATA . '_alias, t1.product_price,t2.' . NV_LANG_DATA . '_title, t1.money_unit
            ...
            if ($result->rowCount()) {
                list($id, $_catid, $product_code, $publtime, $title, $alias, $product_price, $unit, $money_unit) = $result->fetch(3);
                  $data_pro[] = [
                    'id' => $id,
                    'publtime' => $publtime,
                    'title' => $title,
