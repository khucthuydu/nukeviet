...
function format_sizes($data_content, $array_sizes){
    global $lang_module;

    $siteUnit = $array_sizes[$data_content['size_unit']]['title'];
    $label = $lang_module['sizes'] . ' (';
    $result = ' ';

    if($data_content['product_long']){
        $label .= "Dài";
        $result .= $data_content['product_long'] . $siteUnit;
    }
    if($data_content['product_width']){
        if($data_content['product_long']){
            $label .= "/";
            $result .= "/";
        }
        $label .= "Rộng";
        $result .= $data_content['product_width'] . $siteUnit;
    }
    if($data_content['product_height']){
        if($data_content['product_long'] or $data_content['product_width']){
            $label .= "/";
            $result .= "/";
        }
        $label .= "Cao";
        $result .= $data_content['product_height'] . $siteUnit;
    }
...
{
    global $module_info, $lang_module, $module_file, $module_name, $module_upload, $pro_config, $global_config, $global_array_group, $array_wishlist_id, $client_info, $global_array_shops_cat, $meta_property, $pro_config, $user_info, $discounts_config, $my_head, $my_footer, $array_sizes;
    ...
    if (!empty($data_content)) {
        ...
        if (!empty($array_sizes) AND (!empty($data_content['product_long']) or !empty($data_content['product_width']) or !empty($data_content['product_height']))) {
            $xtpl->assign('PRODUCT_SIZE', format_sizes($data_content, $array_sizes));
            $xtpl->parse('main.product_size');
        }
...
{
    global $module_info, $lang_module, $module_config, $module_file, $module_name, $pro_config, $money_config, $global_array_group, $global_array_shops_cat, $discounts, $user_info;
    ...
    if ($pro_config['active_price'] == '1') {
        ...
        if (!empty($discounts) AND !empty($user_info)) {
            $xtpl->assign('DISCOUNT', $discounts);
            $xtpl->parse('main.price3.user_discount');
        }
...
{
    global $module_info, $lang_module, $lang_global, $module_config, $module_data, $module_file, $module_name, $pro_config, $money_config, $global_array_group, $shipping_data, $discounts, $user_info;
    ...
    $xtpl->assign('price_total_before', nv_number_format($price_total - $total_coupons, nv_get_decimals($pro_config['money_unit'])));
    $total = $price_total - $total_coupons;

    $xtpl->assign('price_total', nv_number_format($total * ((100 - $discounts)/100), nv_get_decimals($pro_config['money_unit'])));
    $xtpl->assign('unit_config', $money_config[$pro_config['money_unit']]['symbol']);
...
{
    global $module_info, $lang_module, $module_data, $module_file, $global_config, $module_name, $pro_config, $money_config, $global_array_group, $client_info, $array_location, $array_shops, $discounts, $user_info;
    ...
    $xtpl->assign('URL_ACTIVE_PAY', NV_BASE_SITEURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=ajax&order_id=' . $data_content['order_id'] . $action_pay);
    $xtpl->assign('URL_BACK', NV_BASE_SITEURL . 'index.php?' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=payment&order_id=' . $data_content['order_id']);
    ...
    $xtpl->assign('order_coupons', isset($data_content['coupons']['amount']) ? nv_number_format($data_content['coupons']['amount'], nv_get_decimals($pro_config['money_unit'])) : 0);
    ...
    if ($pro_config['active_price'] == '1') {
        ...
        if (!empty($discounts) AND !empty($user_info)) {
            $xtpl->assign('order_total_before', nv_number_format($data_content['order_total_before'], nv_get_decimals($pro_config['money_unit'])));
            $xtpl->parse('main.price3.user_discount');
        }
...
function history_order($data_content)
{
    ...
    foreach ($data_content as $data_row) {
        ...
        if ($data_row['transaction_status'] == 4) {
            $total_dtt[] = $data_row['order_total']++;
            ...
            $total_ctt[] = $data_row['order_total']++;
    ...
    $xtpl->assign('total_dtt', number_format(array_sum($total_dtt)));
    $xtpl->assign('total_ctt', number_format(array_sum($total_ctt)));
...
{
    global $lang_tmp, $module_name, $module_file, $pro_config, $module_info, $discounts, $user_info;
    ...
    if ($pro_config['active_price'] == '1') {
        if (!empty($discounts) AND !empty($user_info)) {
            $xtpl->assign('TOTAL_BEFORE', $array_data['total_before']);
            $xtpl->assign('DISCOUNT', $discounts);
            $xtpl->parse('main.enable.price.user_discount');
        }
